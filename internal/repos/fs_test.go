...
func TestFileRepo_Delete(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			_, _, _, _, err := r.Index(context.Background())
			assert.NoError(t, err)
...
func TestFileRepo_Index(t *testing.T) {
	...
	t.Run("single id/no index file", func(t *testing.T) {
		_, _, _, _, err := r.Index(context.Background(), "omnicorp-tm-department/omnicorp/omnilamp/subfolder/v0.0.0-20240409155220-80424c65e4e6.tm.json")
		assert.NoError(t, err)
	...
	t.Run("single id/existing index file", func(t *testing.T) {
		_, _, _, _, err := r.Index(context.Background(), "omnicorp-tm-department/omnicorp/omnilamp/subfolder/v3.2.1-20240409155220-3f779458e453.tm.json")
		assert.NoError(t, err)
	...
	t.Run("single id's/index must be sorted", func(t *testing.T) {
		...
		_, _, _, _, err = r.Index(context.Background(), tmId21, tmId12, tmId22, tmId13, tmId11)
		assert.NoError(t, err)
...
func TestFileRepo_Index_Parallel(t *testing.T) {
	...
	for i := 0; i < N; i++ {
		...
		go func(v int) {
			...
			b := make([]byte, 6)
			_, _ = rand.Read(b)
			_, _, _, _, err := r.Index(context.Background(), fmt.Sprintf("author/manuf/mpn/v1.0.%d-%s-%x.tm.json", v, date, b))
			assert.NoError(t, err)
			wg.Done()
		}(i)
	}
...
func TestFileRepo_CheckIntegrity(t *testing.T) {
	...
	t.Run("with only valid ThingModels", func(t *testing.T) {
		...
		_, _, _, _, _ = r.Index(context.Background())
